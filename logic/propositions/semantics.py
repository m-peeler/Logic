...
def truth_values(formula: Formula, models: Iterable[Model]) -> Iterable[bool]:
    ...
    vals = []
    for m in models:
        vals.append(evaluate(formula, m))
    return vals
...
def print_truth_table(formula: Formula) -> None:
    ...
    for v in vars:
        print("| {} ".format(v), end="")
    print("| {} |".format(stringFormula))
    for v in vars:
        print("|" + "-" * (2 + len(v)), end="")
    print("|" + "-"*(2 + len(stringFormula)) + "|")

    for m in models:
        for v in vars:
            print("| {val:<{wid}} ".format(val = "T" if m.get(v) else "F", wid = len(v)), end="")
        print("| {val:<{wid}} |".format(val = "T" if evaluate(formula, m) else "F", wid = len(stringFormula)))
...
def _synthesize_for_model(model: Model) -> Formula:
    ...
    for var, val in model.items():
        if formula is None:
            formula = Formula(var) if val else Formula("~", Formula(var))
        else:
            formula = Formula("&", formula, Formula(var) if val else Formula("~", Formula(var)))
    return formula
...
def synthesize(variables: Sequence[str], values: Iterable[bool]) -> Formula:
    ...
    if formula is None:
        formula = Formula("&", Formula("~", Formula(variables[0])), Formula(variables[0]))
    return formula
...
def _synthesize_for_all_except_model(model: Model) -> Formula:
    ...
    for var, val in model.items():
        if formula is None:
            formula = Formula(var) if not val else Formula("~", Formula(var))
        else:
            formula = Formula("|", formula, Formula(var) if not val else Formula("~", Formula(var)))
    return formula
...
def synthesize_cnf(variables: Sequence[str], values: Iterable[bool]) -> Formula:
    ...
    # Optional Task 2.9
    formula = None
    for model, val in zip(all_models(variables), values): 
        if not val:
            if formula is None:
                formula = _synthesize_for_all_except_model(model)
            else:
