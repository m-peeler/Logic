...
def test_repr(debug=False):
    def test(actu, expt):
        assert actu == expt, ("Incorrect representation - expected " + actu + ", reieved " + expt) if debug else ""
    if debug:
        print("Testing representation of formula 'x12'")
    test(str(Formula('x12')), 'x12')
    if debug:
        print("Testing representation of formula 'T'")
    test(str(Formula('T')), 'T')
    if debug:
        print("Testing representation of formula '~~F'")
    test(str(Formula('~', Formula('~', Formula('F')))), '~~F')
    if debug:
        print("Testing representation of formula '(p|p)'")
    test(str(Formula('|',Formula('p'),Formula('p'))), '(p|p)')
    if debug:
        print("Testing representation of formula '~(p&q7)'")
    test(str(Formula('~', Formula('&', Formula('p'), Formula('q7')))), \
           '~(p&q7)')
    if debug:
        print("Testing representation of formula '((p->q)->(~q->~p))'")
    test(str(Formula('->', Formula('->', Formula('p'), Formula('q')), \
...
def test_polish(debug=False):
    ...
    test1, actual1 = Formula('x12').polish(), "x12"
    assert test1 == actual1, "Incorrect parsing - " + str(test1) + " should be " + str(actual1)
    if debug:
        print("Testing polish of formula '|pp' (in infix: '(p|p)')")
    test2, actual2 = Formula('|', Formula('p'), Formula('p')).polish(), "|pp"
    assert test2 == actual2, "Incorrect parsing - " + str(test2) + " should be " + str(actual2)
    if debug:
        print("Testing polish of formula '~&pq7' (in infix: '~(p&q7)')")
    test3, actual3 = Formula('~', Formula('&', Formula('p'), Formula('q7'))).polish(), "~&pq7"
    assert test3 == actual3, "Incorrect parsing - " + str(test3) + " should be " + str(actual3) 

def test_parse_polish(debug=False):
    for polish in ['p', '~x12', '&xy', '~~|x~T', '|&x1~x2F']:
        if debug:
            print("Testing polish parsing of formula", polish)
        parsed_polish = Formula.parse_polish(polish).polish()
        assert parsed_polish == polish, "Incorrect parsing - " + str(parsed_polish) + " is not " + polish

# Tests for Chapter 3

def test_repr_all_operators(debug=False):
    def assertMatch(actual, expected):
        assert actual == expected, actual + " is not correct."
    if debug:
        print("Testing representation of formula '(x12+x12)'")
    assertMatch(str(Formula('+',Formula('x12'),Formula('x12'))), \
        '(x12+x12)')
    if debug:
        print("Testing representation of formula '(T-|F)'")
    assertMatch(str(Formula('-|',Formula('T'),Formula('F'))), '(T-|F)')
    if debug:
        print("Testing representation of formula '(p-&p)'")
    assertMatch(str(Formula('-&',Formula('p'),Formula('p'))), '(p-&p)')
    if debug:
        print("Testing representation of formula '(p<->p)'")
    assertMatch(str(Formula('<->',Formula('p'),Formula('p'))), '(p<->p)')
    if debug:
        print("Testing representation of formula '(p<->~p)'")
    assertMatch(str(Formula('<->',Formula('p'),Formula('~',Formula('p')))), '(p<->~p)')
    if debug:
        print("Testing representation of formula '~(p~&q7)'")
    assertMatch(str(Formula('~', Formula('-&', Formula('p'), Formula('q7')))), '~(p-&q7)')
    if debug:
        print("Testing representation of formula '(~(p+q)<->(~q<->~p))'")
    assertMatch(str(Formula("<->", Formula('~', Formula("+", Formula("p"), Formula("q"))), \
                              Formula("<->", Formula("~", Formula("q")), Formula("~", Formula("p"))))), \
           '(~(p+q)<->(~q<->~p))')
    if debug:
        print("Testing representation of formula '(~(p1+q)|(~q-&~p))'")
    assertMatch(str(Formula("|", Formula('~', Formula("+", Formula("p1"), Formula("q"))), \
                              Formula("-&", Formula("~", Formula("q")), Formula("~", Formula("p"))))), \
           '(~(p1+q)|(~q-&~p))')
